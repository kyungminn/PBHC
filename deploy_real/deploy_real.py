...
class Controller:
    ...
    def run(self):
        ...
        # Clip observations to match training (clip_observations: 100.0)
        self.obs_buf = torch.clamp(self.obs_buf, -100.0, 100.0)
        ...
        if self.is_student_model:
            ...
            # Clip observations to match training (clip_observations: 100.0)
            # Note: Only actor_obs is clipped in sim-to-sim (urcirobot.py line 321-324)
            clip_obs = 100.0
            actor_obs = np.clip(actor_obs, -clip_obs, clip_obs)
